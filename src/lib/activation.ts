...
/**
 * Administrative manual activation of an account.
 * Automatically covers activation fee grant if fund wallet is insufficient,
 * places member into Tier 0 queue, activates status, and pays sponsor direct commission.
 */
export async function adminActivateUserAccount(userId: string): Promise<ActivationResult> {
  return await db.$transaction(async (tx) => {
    // 1. Fetch user
    const user = await tx.user.findUnique({
      where: { id: userId },
    ...
    // If fund balance is less than $10, Admin grants the required fee to Fund Wallet
    if (fundBalanceDec.lessThan(entryFee)) {
      const grantAmount = entryFee.minus(fundBalanceDec);
      await executeLedgerTransaction(
        {
          userId: user.id,
          ...
          wallet: "FUND",
          amount: grantAmount,
          referenceKey: `ADMIN_ACTIVATION_GRANT_${user.id}_${Date.now()}`,
          description: "Administrative Fund Grant for Account Activation",
        },
        tx
      );
    }
    ...
    await executeLedgerTransaction(
      {
        ...
        amount: entryFee.negated(),
        referenceKey: activationRefKey,
        description: "$10 USDT Micro-Entry Account Activation Fee (Admin Action)",
      },
      tx
    );
    ...
    if (user.sponsorId) {
      ...
      // Only pay direct commission if sponsor is not banned
      if (sponsor && sponsor.status !== "BLOCKED" && sponsor.status !== "SUSPENDED") {
        const directBonus = entryFee.times(RATES.DIRECT_COMMISSION_PERCENT).dividedBy(100);
        const sponsorRefKey = `DIRECT_COMMISSION_FROM_${user.id}_FOR_${user.sponsorId}`;
    ...
    if (!existingEntry) {
      ...
      tier0Index = existingEntry.queueIndex;
    }
